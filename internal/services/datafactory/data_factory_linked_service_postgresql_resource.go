...
func resourceDataFactoryLinkedServicePostgreSQLCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if _, err := client.CreateOrUpdate(ctx, id.ResourceGroup, id.FactoryName, id.Name, linkedService, ""); err != nil {
		return fmt.Errorf("creating/updating Data Factory PostgreSQL %s: %+v", id, err)
	}
...
func resourceDataFactoryLinkedServicePostgreSQLRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("retrieving Data Factory PostgreSQL %s: %+v", *id, err)
	}
	...
	if !ok {
		return fmt.Errorf("classifying Data Factory Linked Service PostgreSQL %q (Data Factory %q / Resource Group %q): Expected: %q Received: %q", id.Name, id.FactoryName, id.ResourceGroup, datafactory.TypeBasicLinkedServiceTypePostgreSQL, *resp.Type)
	}
