...
func resourceDataFactoryLinkedServiceSynapse() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		...
		Importer: pluginsdk.ImporterValidatingResourceIdThen(func(id string) error {
			...
			return err
		}, importDataFactoryLinkedService(datafactory.TypeBasicLinkedServiceTypeAzureSQLDW)),
...
func resourceDataFactoryLinkedServiceSynapseCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
		...
		if !meta.(*clients.Client).Features.SkipExistenceCheckAndAllowOverwrite {
			...
			if err != nil {
				if !utils.ResponseWasNotFound(existing.Response) {
					return fmt.Errorf("checking for presence of existing Data Factory Synapse %s: %+v", id, err)
				}
			}
	...
	if _, err := client.CreateOrUpdate(ctx, id.ResourceGroup, id.FactoryName, id.Name, linkedService, ""); err != nil {
		return fmt.Errorf("creating/updating Data Factory Synapse %s: %+v", id, err)
	}
...
func resourceDataFactoryLinkedServiceSynapseRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		...
		return fmt.Errorf("retrieving Data Factory Synapse %s: %+v", *id, err)
	}
	...
	if !ok {
		return fmt.Errorf("classifying Data Factory Synapse %s: Expected: %q Received: %q", *id, datafactory.TypeBasicLinkedServiceTypeAzureSQLDW, *resp.Type)
	}
...
func resourceDataFactoryLinkedServiceSynapseDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if err != nil {
		if !utils.ResponseWasNotFound(response) {
			return fmt.Errorf("deleting Data Factory Synapse %s: %+v", *id, err)
		}
	}
